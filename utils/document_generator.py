...
class DocumentGenerator:
	...
	def _generate_deviation_statement(self) -> str:
		...
		# Column name detection for flexibility across input files
		item_names = ['Item', 'Item No', 'Item No.', 'Item Number', 'ItemNo', 'Sr No', 'Sr. No', 'S.No', 'S. No.']
		desc_names = ['Description', 'Item Description', 'Desc']
		unit_names = ['Unit', 'Units']
		qty_names = ['Quantity', 'Qty', 'Qty.', 'Executed Quantity', 'Qty Executed', 'Measurement']
		rate_names = ['Rate', 'Unit Rate', 'Unit Price', 'Price', 'Rate (Rs.)']
		
		wo_item_col = self._find_column(self.work_order_data, item_names) or 'Item'
		wo_desc_col = self._find_column(self.work_order_data, desc_names) or 'Description'
		wo_unit_col = self._find_column(self.work_order_data, unit_names) or 'Unit'
		wo_qty_col = self._find_column(self.work_order_data, qty_names) or 'Quantity'
		wo_rate_col = self._find_column(self.work_order_data, rate_names) or 'Rate'
		
		bq_item_col = self._find_column(self.bill_quantity_data, item_names) or wo_item_col
		bq_qty_col = self._find_column(self.bill_quantity_data, qty_names) or 'Quantity'
		...
		for index, wo_row in self.work_order_data.iterrows():
			...
			bq_row = None
			if not self.bill_quantity_data.empty and bq_item_col in self.bill_quantity_data.columns:
				item_value = wo_row.get(wo_item_col)
				if pd.notna(item_value):
					matching_rows = self.bill_quantity_data[self.bill_quantity_data[bq_item_col] == item_value]
					if not matching_rows.empty:
						bq_row = matching_rows.iloc[0]
			
			wo_qty = self._safe_float(wo_row.get(wo_qty_col, 0))
			wo_rate = self._safe_float(wo_row.get(wo_rate_col, 0))
			wo_amount = wo_qty * wo_rate
			
			bq_qty = self._safe_float(bq_row.get(bq_qty_col, 0)) if bq_row is not None else 0.0
			bq_amount = bq_qty * wo_rate
			
			excess_qty = max(0.0, bq_qty - wo_qty)
			excess_amt = excess_qty * wo_rate
			saving_qty = max(0.0, wo_qty - bq_qty)
			saving_amt = saving_qty * wo_rate
			
			html_content += f"""
					<tr>
						<td>{wo_row.get(wo_item_col, '')}</td>
						<td>{wo_row.get(wo_desc_col, '')}</td>
						<td>{wo_row.get(wo_unit_col, '')}</td>
	...
	def _generate_extra_items_statement(self) -> str:
		...
		if not self.extra_items_data.empty:
			...
			# Column detection for extra items
			item_col = self._find_column(self.extra_items_data, ['Item', 'Item No', 'Item No.']) or 'Item No'
			desc_col = self._find_column(self.extra_items_data, ['Description', 'Item Description']) or 'Description'
			unit_col = self._find_column(self.extra_items_data, ['Unit', 'Units']) or 'Unit'
			qty_col = self._find_column(self.extra_items_data, ['Quantity', 'Qty', 'Qty.']) or 'Quantity'
			rate_col = self._find_column(self.extra_items_data, ['Rate', 'Unit Rate', 'Unit Price']) or 'Rate'
			
			total_amount = 0
			for index, row in self.extra_items_data.iterrows():
				qty = self._safe_float(row.get(qty_col, 0))
				rate = self._safe_float(row.get(rate_col, 0))
				amount = qty * rate
				total_amount += amount
				
				html_content += f"""
						<tr>
							<td>{row.get(item_col, '')}</td>
							<td>{row.get(desc_col, '')}</td>
							<td>{row.get(unit_col, '')}</td>
							<td class="amount">{qty:.0f}</td>
							<td class="amount">{rate:.0f}</td>
